      ...
      <section id="business" className="mx-auto max-w-7xl scroll-mt-28 px-6 py-16">
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          <Reveal className="flex flex-col gap-5">
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-cyan-400 text-black">
          ...
          <Reveal delay={0.1} className="glass relative overflow-hidden rounded-3xl p-8">
            <div className="animate-blob absolute -top-10 -right-10 h-48 w-48 rounded-full bg-cyan-400/20 blur-3xl" />
            ...
            <Reveal key={tier.name}>
              <div
                className={`relative flex h-full flex-col gap-6 rounded-3xl p-7 ${
                  tier.highlighted
                    ? 'border-2 border-cyan-400/40 bg-gradient-to-b from-cyan-400/10 to-transparent'
                    : 'glass'
                }`}
              >
                {tier.highlighted && (
                  <span className="absolute -top-3 left-7 flex items-center gap-1 rounded-full bg-gradient-to-r from-violet-500 to-cyan-400 px-3 py-1 text-xs font-semibold text-black">
                    <Sparkles className="h-3 w-3" /> {tier.tag}
                  </span>
                )}
                <div>
                  {!tier.highlighted && <p className="mb-1 text-xs text-white/40">{tier.tag}</p>}
        ...
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          <Reveal delay={0.1} className="order-2 md:order-1">
            <div className="glass relative overflow-hidden rounded-3xl p-8">
              <div className="animate-blob absolute -bottom-10 -left-10 h-48 w-48 rounded-full bg-emerald-400/20 blur-3xl [animation-delay:4s]" />
              <div className="relative flex flex-col gap-4">
                <p className="text-xs text-white/40">Domains we cover</p>
                <div className="flex flex-wrap gap-2">
                  {STUDENT_DOMAINS.map((d) => (
                    <span key={d} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/70">
                      {d}
                    </span>
                  ))}
                </div>
                <p className="mt-2 text-xs text-white/40">What's included</p>
          ...
          <Reveal className="order-1 flex flex-col gap-5 md:order-2">
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-400 to-emerald-400 text-black">
              <Boxes className="h-6 w-6" />
            </span>
            <h2 className="font-display text-3xl font-bold text-white sm:text-4xl">Student tech projects</h2>
            <p className="text-white/60">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              {[
                ...
                <div key={p.label} className="glass rounded-2xl p-4">
                  <p className="text-xs text-white/45">{p.label}</p>
                  <p className="font-display mt-1 font-bold text-white">{p.price}</p>
                </div>
              ))}
            </div>
            <Link
              to="/contact"
              className="mt-2 inline-flex w-fit items-center gap-2 rounded-full bg-gradient-to-r from-cyan-400 to-emerald-400 px-6 py-3 text-sm font-semibold text-black transition-transform hover:scale-105"
            >
